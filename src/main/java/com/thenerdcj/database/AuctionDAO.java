...
public class AuctionDAO {
    ...
    public CompletableFuture<List<Auction>> getActiveAuctions() {
        return dbOps.supplyAsync(() -> {
            ...
            try (Connection conn = dbOps.getPlugin().getDatabaseManager().getConnection(); // temporary bridge during migration
                 PreparedStatement ps = conn.prepareStatement("SELECT * FROM auctions WHERE sold = 0")) {
