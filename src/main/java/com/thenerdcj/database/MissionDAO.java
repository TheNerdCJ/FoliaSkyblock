            ...
            try {
                return withConnection(conn -> {
                    try (PreparedStatement ps = conn.prepareStatement(
                        ...
                        while (rs.next()) {
                            // Full reconstruction using the load ctor from Mission. Use ItemSerializer for any item rewards.
                            String rewardItemBase64 = rs.getString("reward_item_base64");
                            // (ItemSerializer can be used by callers or here if needed for immediate deserialize in future)
