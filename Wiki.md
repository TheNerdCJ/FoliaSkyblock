...
- **vs IridiumSkyblock**: Similar missions/shop/upgrades/team. FoliaSkyblock superior in native Folia schedulers (large scale no lag), custom procedural gen (unique per island, no external deps), explicit party XP balance + config, built-in Neural+heuristic AC (Iridium relies external), dual economy explicit + hardened, per-dim reset without main, museum/slayer/minion depth, PAPI + DB paginated tops. Pure procedural (no schematic support).
